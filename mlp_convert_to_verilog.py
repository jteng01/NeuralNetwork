...
def write_module_definition(weights, biases):
    INPUTS = len(weights[0][0])  # number of inputs to first layer (e.g. 9)
    OUTPUTS = len(weights[-1])   # number of neurons in last layer (e.g. 2)
...
def write_parameters(weights, biases):
    code = ""
    # For each layer, write weights and biases
    for layer_idx in range(len(weights)):
        ...
        for neuron_idx in range(n_neurons):
            # weights plus bias weight? No — weights shape is neuron x inputs only
            # careful with comma after last neuron
            line = "        '{"
